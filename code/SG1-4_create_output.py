...
x_test2 = np.load(f'./SG_LGR{level}_val_input2.npz')['input'][:nval].astype(np.float32)
...
y_test = np.moveaxis(np.load(f'./SG_LGR{level}_test_output.npz')['output'][:nval], 4, 1).astype(np.float32)
...
x_test2 = np.load(f'./SG_LGR{level}_test_input2.npz')['input'].astype(np.float32)
# x_test2 = np.load(f'SG_LGR{level}_test_input2_sequential.npz')['arr_0'].astype(np.float32)
x_test2 = (x_test2 - mean_)/(std_)
# x_test = np.concatenate([x_test1, x_test2], axis=-1)
...
if level == 1:    
    ...
    np.savez(f'SG_LGR{level+1}_test_input2_sequential.npz', next_level_input)
    np.savez(f'SG{level}_test_pred.npz', current_model_output)
    ...
    np.savez(f'SG{level}_test_pred.npz', current_model_output)
    if level != 4:
        np.savez(f'SG_LGR{level+1}_test_input2_sequential.npz', current_model_output)
