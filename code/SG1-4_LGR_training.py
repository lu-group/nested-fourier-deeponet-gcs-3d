...
level = 1 #Adjust this to the level you want to train
...
x_train2 = np.load(f'./SG_LGR{level}_train_input2.npz')['input'][:ntrain].astype(np.float32)
x_train2 = (x_train2 - mean_)/(std_ + eps)
x_train = np.concatenate([x_train1, x_train2], axis=-1)
x_train_MIO = np.load(f'./dP_LGR{level}_train_input1.npz')['input'][:ntrain, 0, 0, 0, 5:6].astype(np.float32)
x_train = (x_train, x_train_MIO, t)
y_train = np.moveaxis(np.load(f'./SG_LGR{level}_train_output.npz')['output'][:ntrain], 4, 1).astype(np.float32)
...
y_test = np.moveaxis(np.load(f'./SG_LGR{level}_val_output.npz')['output'][-nval:], 4, 1).astype(np.float32)
